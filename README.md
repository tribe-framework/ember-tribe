...
The addon automatically configures the following essential packages:
...
It enables rapid application development by:

- Providing a pre-configured addon ecosystem for common functionality
...
| ----------------- | ------------------------------------- |
| `routes`          | `ember g route <n>`                |
| `routes`          | `ember g controller <n>`           |
| `components`      | `ember g component <n> -gc`        |
| `services`        | `ember g service <n>`              |
| `helpers`         | `ember g helper <n>`               |
| `modifiers`       | `ember g modifier <n>`             |
...
`config/storylang.json` is a structured configuration file used in the ember-tribe ecosystem to define the frontend implementation of your application. It works in conjunction with `simplified-types.json` (which defines your data types) to create a complete frontend specification.

It serves as a blueprint for frontend developers to understand:
...
- `type`: Type from the built-in components list (see below)
...
**Local Helpers** — defined as methods within component classes, scoped to a specific component, called with `this.` prefix in templates:
...
**Global Helpers** — defined in `app/helpers/` as separate files, available across all templates:
...
**Component–Modifier Pattern** (component manages state, modifier handles DOM):
...
Smart use of EmberData can significantly reduce the size of the codebase. All backend data uses the `modules` key for field access, except for `.id` and `.slug`. All field names from backend storage use underscore notation: `modules.any_field`.
...
Prefer using backend (`this.store.query`) for search, filter and sort, over front-end JS functions. Avoid using `this.store.findAll` altogether; use `this.store.query` with `page: { offset:0, limit:-1 }` instead.
